  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (previousSessionRef.current) {
        try { await gamesAPI.stopSession(previousSessionRef.current); } catch (e) { /* ignore */ }
      ...
      if (activeGame !== 'bubble') {
        try {
          ...
          previousSessionRef.current = sid;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg border border-gray-100 dark:border-gray-700">
      ...
      {activeGame === 'bubble' && (
        <BubblePopGame
          ...
          onStart={async () => {
            try {
              const sid = await gamesAPI.startSession(userId, 'bubble');
              previousSessionRef.current = sid;
              setSessionId(sid);
            } catch (e) { /* ignore */ }
          }}
          onStop={async (finalScore) => {
            try {
              const sid = previousSessionRef.current || sessionId;
              if (sid) await gamesAPI.stopSession(sid);
              previousSessionRef.current = null;
            } catch (e) { /* ignore */ }
...
const BubblePopGame = ({ difficulty, themeClass, onPop, onStart, onStop, highScore }) => {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(id);
  }, [isPlaying, onStop, score, timeLeft]);
...
const ShapeSortingGame = ({ difficulty, themeClass, onPlace }) => {
  ...
  const shapeTypes = useMemo(() => ['circle', 'square', 'triangle'], []);
