...
import { Calendar, TrendingUp, Save, Edit3, Sparkles, BarChart3, AlertCircle, RefreshCw } from 'lucide-react';
import Reveal from '../components/Reveal';
import api from '../services/api';
import { useUser } from '../contexts/UserContext';
import MoodTrendChart from '../components/MoodTrendChart';
import SentimentCard from '../components/SentimentCard';
import { journalAPI, sentimentAPI } from '../services/api';
...
const Journal = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      const saved = await journalAPI.addEntry({
        content: currentEntry.trim(),
        mood: selectedMood,
        sentiment,
      });
      setRecentEntries((prev) => [saved, ...prev].slice(0, 20));
      setCurrentEntry('');
      setSelectedMood('');
      setSentiment(null);
    } catch (error) {
      setSentimentError(error.message || 'Unable to save journal entry');
    }
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await journalAPI.getEntries(20);
        if (mounted) setRecentEntries(rows);
      } catch {
              ...
              <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={!currentEntry.trim() || isSentimentLoading}
                    className={`px-4 py-3 rounded-xl font-medium transition-all duration-300 ${
                      currentEntry.trim() && !isSentimentLoading
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-gray-400">
                      {String(entry.content || '').split(' ').filter(Boolean).length} words
                    </span>
                  </div>
