...
import { positivityAPI, recoAPI, recommendationsAPI } from '../services/api';
...
const PositivityDrops = () => {
  ...
  const categories = [
    ...
    ...Array.from(new Set(content.map((c) => c.tags?.[0]).filter(Boolean))).map((tag) => ({
      value: tag,
      label: tag,
      icon: '✨',
    })),
  ...
  const filteredDrops = selectedCategory === 'all'
    ? content.filter((c) => c.contentType === 'quote')
    : content.filter((c) => c.contentType === 'quote' && Array.isArray(c.tags) && c.tags.includes(selectedCategory));

  const quickAffirmations = content
    .filter((c) => c.contentType === 'affirmation')
    .map((c) => c.text)
    .slice(0, 8);
  ...
  const toggleFavorite = (id) => {
    setFavorites((prev) => {
      ...
      const action = prev.includes(id) ? 'view' : 'favorite';
      positivityAPI.trackInteraction({ contentId: id, action }).catch(() => {});
      return next;
    });
  };
  ...
  const copyToClipboard = (text) => {
    ...
    const target = filteredDrops[currentQuote];
    if (target?._id) {
      positivityAPI.trackInteraction({ contentId: target._id, action: 'copy' }).catch(() => {});
    }
  };
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [positivityRows, catalog] = await Promise.all([
          positivityAPI.listContent({ limit: 200 }),
          recommendationsAPI.getCatalog({ type: 'book', limit: 6 }),
        ]);
        setContent(positivityRows);
        setReadingItems(catalog.slice(0, 3));
      } catch {
        setContent([]);
        setReadingItems([]);
      }
            ...
            <div className="relative z-10">
              <div className="flex items-start justify-between mb-6">
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => toggleFavorite(currentDrop._id)}
                    className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
                      favorites.includes(currentDrop._id)
                        ? 'bg-red-500 text-white scale-110'
                        : 'bg-white/20 hover:bg-white/30'
                    }`}
                  >
                    <Heart className="w-5 h-5" fill={favorites.includes(currentDrop._id) ? 'currentColor' : 'none'} />
                  </button>
                  <button
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-lg font-semibold opacity-90">— {currentDrop.author || 'Anonymous'}</p>
                  <p className="text-sm opacity-75">{currentDrop.tags?.[0] || 'General'}</p>
                </div>
