...
import { reflectionsAPI, sentimentAPI } from '../services/api';
...
const ReflectionWall = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await reflectionsAPI.getReflections('all', 100);
        if (mounted) setReflections(rows);
      } catch {
  ...
  const handlePostReflection = async () => {
    ...
    try {
      const created = await reflectionsAPI.addReflection({
      ...
      setSentimentError(error.message || 'Unable to post reflection');
    }
  };
  ...
  const handleReaction = async (reflectionId, reactionType) => {
    try {
      const updated = await reflectionsAPI.addReaction({ reflectionId, reaction: reactionType });
      setReflections((prev) =>
        prev.map((row) => (row.id === reflectionId ? { ...row, reactions: updated.reactions } : row))
      );
    } catch {
      // ignore reaction failures to avoid blocking feed interactions
    }
  };
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <div className="flex space-x-3">
                <button
                  ...
                  disabled={!newReflection.trim() || isSentimentLoading}
                  className={`px-4 py-2 bg-white/20 rounded-lg hover:bg-white/30 transition-colors ${!newReflection.trim() || isSentimentLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
              ...
              <div className="mt-4 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
                <div className="flex items-start space-x-2">
                  ...
                  <div className="flex-1">
                    ...
                    >
                      <RefreshCw className={`w-3 h-3 ${isSentimentLoading ? 'animate-spin' : ''}`} />
                      <span>Retry</span>
                    </button>
                  </div>
                </div>
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{formatTimeAgo(reflection.createdAt)}</p>
                </div>
              </div>
