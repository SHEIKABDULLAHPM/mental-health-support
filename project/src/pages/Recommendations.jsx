...
import { Book, Music, Globe, Heart, Star, AlertCircle, RefreshCw } from 'lucide-react';
...
const Recommendations = () => {
  ...
  const filteredRecs = recs.filter((r) => {
    ...
    if (activeTab === 'books') return t === 'book';
    if (activeTab === 'music') return t === 'music' || t === 'nature-sound';
    return true;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700">
        ...
        {!error && recs.length === 0 && !loading && (
          <p className="text-center text-gray-500 dark:text-gray-400 py-8">
            No backend recommendations available for this selection yet.
          </p>
        )}
      ...
      {/* Backend-driven tab view */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredRecs.map((item, index) => (
          <Reveal key={`${item.item_id}-${index}`} animation="fade-in" delay={index * 70} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div className="p-6">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center space-x-1 text-yellow-500">
                  ...
                  <span className="text-sm font-medium">{item.score?.toFixed ? item.score.toFixed(2) : item.score}</span>
                </div>
              </div>

              <p className="text-gray-600 dark:text-gray-300 text-sm mb-4 leading-relaxed">
                {Array.isArray(item.tags) && item.tags.length ? item.tags.join(', ') : 'Personalized recommendation'}
              </p>

              <button className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white py-3 px-4 rounded-xl font-medium hover:from-blue-600 hover:to-purple-700 transition-all duration-300 hover:scale-105">
                Open
              </button>
            </div>
          </Reveal>
